...
1. 业务平台可先查询算法目录和目标算法版本 Schema。
2. 业务平台调用 `POST /v1/executions?validate_only=true` 提交完整 Flow DSL 做预校验。
3. 算法平台校验 DSL 结构、算法版本引用、映射规则及 DAG 无环约束。
4. 预校验通过后，业务平台再调用 `POST /v1/executions`，并携带 `Idempotency-Key`。
5. 算法平台创建 `execution_id` 并立即返回（异步受理）。
6. 编排器解析就绪节点并投递执行任务。
7. Worker 执行节点并持久化节点输出快照。
8. 首个节点失败时：节点置为 `FAILED`，流程置为 `FAILED`，停止下游调度。
9. 流程结束（`SUCCEEDED` 或 `FAILED`）后发送签名回调；轮询查询持续可用作为兜底。

## 6. 对外 API 契约（算法平台）

- `GET /v1/algorithms`
  - 返回分组结构 `categories[].algorithms[]`，包含分类/状态/最新版本。
- `GET /v1/algorithms/{algo_code}/versions/{version}`
  - 返回 `input_schema`、`output_schema` 与运行默认配置。
- `POST /v1/executions?validate_only=true`
  - 仅校验，不创建执行实例，返回校验结果。
- `POST /v1/executions`
  - 提交完整 Flow DSL 异步执行；返回 `execution_id` 与初始状态。
  - 需支持 `Idempotency-Key`；同键语义冲突返回 `409 IDEMPOTENCY_CONFLICT`。
- `GET /v1/executions/{execution_id}`
  - 返回流程级状态与摘要。
- `GET /v1/executions/{execution_id}/nodes`
  - 返回节点级状态与快照摘要。
- `POST /v1/executions/{execution_id}/retry`
  - 仅 `FAILED` 状态可手工重试，返回新 `execution_id` 与 `parent_execution_id`。
  - 非 `FAILED` 返回 `409 RETRY_NOT_ALLOWED`。

业务平台回调端点：
- `POST <business_callback_url>`
  - 回调签名载荷，包含 `execution_id`、最终状态、摘要、时间戳。
  - 头部包含：`X-Signature`（`HMAC-SHA256`）、`X-Timestamp`、`X-Trace-Id`。
  - 时间戳容忍窗口：5 分钟（防重放）。
...
- 校验期错误：返回 `400`，并提供机器可读错误码（`DSL_SCHEMA_INVALID`、`FLOW_GRAPH_CYCLE`、`ALGO_VERSION_NOT_FOUND`）。
- 运行期节点错误：统一错误类型并记录堆栈。
- 超时：转换为 `NODE_TIMEOUT` 并使流程失败。
- 回调安全：`HMAC-SHA256` 签名 + 时间戳校验（5 分钟窗口）。
- 回调失败：异步重试（3 次指数退避）并保留轮询兜底。
- 幂等性：提交接口支持 `Idempotency-Key`，去重窗口 24 小时。
- 幂等语义冲突：返回 `409 IDEMPOTENCY_CONFLICT`。
- 重试约束：仅 `FAILED` 执行可重试，否则返回 `409 RETRY_NOT_ALLOWED`。
...
- 集成测试：
  ...
  - 回调重试路径
- 契约测试：
  - `validate_only` 预校验 + 正式提交链路
  - 幂等冲突行为（`409 IDEMPOTENCY_CONFLICT`）
  - 重试约束（仅 `FAILED`）与 `parent_execution_id` 关联
